    ...
    def fetch_weights(self):
        with TaskLog("fetch vgg16 weights"):
            ...
            if os.path.exists(f"{WEIGHT_DIR}/vgg16.pth"):
                return
            torch_vgg = torchvision.models.vgg16(
    ...
    def load_weights(self):
        with TaskLog("load vgg16 weights"):
            self.network.load_state_dict(
                torch.load(f"{WEIGHT_DIR}/vgg16.pth", weights_only=True)
            )
            ...
            for epoch in range(NUM_EPOCHS):
                ...
                for layer in CONTENT_LAYERS:
                    content_loss += F.mse_loss(
                        target_features[layer], original_features[layer]
                    )
                ...
                total_loss = (
                    CONTENT_COEFFICIENT * content_loss + STYLE_COEFFICIENT * style_loss
                )
